 ...
 * Accounts for the 50% padding around the image in zoom mode
 */
export function calculateVisibleImageArea(viewport: ViewportInfo): VisibleArea {
  ...
  // The image starts at 50% padding offset in zoom mode
  const imageOffsetX = imageDimensions.width * 0.5
  const imageOffsetY = imageDimensions.height * 0.5
 ...
 */
export function calculateOptimalLayerPositioning(
  input: LayerPositioningInput
): LayerPositioning {
  const {
  ...
  const position = calculateLayerPositionInArea(
    targetArea,
    layerSize,
    positioning
  )
  ...
  if (needsCoordinateConversion && viewport) {
    return convertPreviewToOutputCoordinates(
      result,
