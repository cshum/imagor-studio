...
func init() {
	Migrations.MustRegister(func(ctx context.Context, db *bun.DB) error {
		...
		if _, err := db.NewCreateIndex().
			Model((*ProcessingUsage)(nil)).
			Index("idx_processing_usage_space_bucket").
			Column("space_id", "bucket_start_at").
			IfNotExists().
			Exec(ctx); err != nil {
			return err
		}
		if _, err := db.NewCreateIndex().
			Model((*ProcessingUsage)(nil)).
			Index("idx_processing_usage_org_bucket").
			Column("org_id", "bucket_start_at").
			IfNotExists().
			Exec(ctx); err != nil {
			return err
		}
		...
		if _, err := db.NewCreateIndex().
			Model((*ProcessingUsageBatch)(nil)).
			Index("idx_processing_usage_batches_received_at").
			Column("received_at").
			IfNotExists().
			Exec(ctx); err != nil {
			return err
