...
type SpaceS3Loader struct {
	store      *spaceconfigstore.SpaceConfigStore
...
func (l *SpaceS3Loader) Get(r *http.Request, image string) (*imagor.Blob, error) {
	...
	if cfg.Suspended {
		return nil, fmt.Errorf("space %q is suspended", cfg.Key)
	}
	...
	objectKey := cfg.Prefix + image

	blob := imagor.NewBlob(func() (io.ReadCloser, int64, error) {
		client, err := buildS3Client(r.Context(), cfg)
		if err != nil {
			return nil, -1, fmt.Errorf("build S3 client for space %q: %w", cfg.Key, err)
		}
...
func (l *SpaceS3Loader) resolveSpace(host string) (*spaceconfigstore.SpaceConfig, error) {
...
func buildS3Client(ctx context.Context, cfg *spaceconfigstore.SpaceConfig) (*s3.Client, error) {
