...
func NewSESEmailSender(region, fromEmail, appBaseURL, appAPIBaseURL string) (EmailSender, error) {
	...
	return &sesEmailSender{
		...
		fromEmail: fromEmail,
		loginURL:  apiBase + "/api/auth/google/login",
	}, nil
}

func (s *sesEmailSender) SendSpaceInvitation(ctx context.Context, params EmailParams) error {
	...
	_, err := s.client.SendEmail(ctx, &sesv2.SendEmailInput{
		...
		Destination: &types.Destination{
			ToAddresses: []string{params.ToEmail},
		},
